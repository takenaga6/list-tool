...
# 従業員数の抽出パターン（B: 歩留まり改善 2026-06-19）
# 旧パターンはカウンタ「名」限定・カンマ非対応・「約」非対応で、
# 「従業員数 50人」「1,200名」「約80名」「人員30名」等を取りこぼしていた。
# 本番ログで「従業員数だけ落ち」が約2.4万件＝歩留まり最大のボトルネックだったため拡張。
#   - カウンタ: 名 / 人 の両方
#   - 数字: カンマ区切り可（[\d,]+。extract_employee_count でカンマ除去）
#   - 「約」を許容
#   - ラベル: 従業員 / 社員 / スタッフ / 職員 / 人員、総勢・総従業員数・グループ従業員数
EMPLOYEE_PATTERNS = [
