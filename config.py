...
NG_INDUSTRY_KEYWORDS = [
    ...
    # Phase 6.2B.1: フィジカルケア・運動系の直接競合（出張・派遣型）
    # Well Body はオフィス出張ストレッチのため、同様の出張・派遣型が直接競合
    "出張ストレッチ", "オフィスストレッチ", "派遣ストレッチ",
    "出張マッサージ", "オフィスマッサージ", "派遣マッサージ",
    "訪問マッサージ", "企業向けマッサージ",
    "出張ヨガ", "オフィスヨガ",
    "出張フィットネス", "オフィスフィットネス", "企業向けフィットネス",
    # Phase 6.2B.1: 整体・整骨系（規模フィルタで自然除外されるが明示で安全性向上）
    "整体院", "整骨院", "接骨院",
    "カイロプラクティック", "リフレクソロジー",
]
...
# 2026-06-26 事業判断: 「広告代理店・総合広告・PR会社・マーケティング会社」の4語を
# NGから除外（営業対象として通す）。残るメディア系5語は引き続きNG。
...
INDUSTRY_PROFIT_MEDIUM_KEYWORDS = [
    "受託開発", "システム開発", "SI", "SIer",
    "システムインテグレ",
    # 2026-06-26 事業判断: 「総合商社」を必須NGから撤廃（商社/卸売カテゴリは
    # 加点対象であり、NG扱いとの内部矛盾を解消。減点は付かない=medium-high分類）
]
...
#
# 【設計方針】
# このツールはアウトバウンド営業リスト生成ツールのため、
# 上場企業本体は「購買窓口到達が困難」としてNG、
# プライム上場企業の子会社のみ「親の信用力×トップ即決」として例外通過する。
#
# 【厳格化の理由（2026-05-20）】
# 旧版に含まれていた "100%子会社" / "完全子会社" / "グループ会社" /
# "の連結子会社" / "の関連会社" は親会社の上場ステータスを問わないため、
# 非上場親会社や東証スタンダード・グロース上場親会社の子会社も
# True と判定してしまう誤検出が発生していた。
# → これらを削除し、プライム市場上場を示す表現のみ残す。
#
# 【残したキーワードの根拠】
# - "東証プライム上場の" / "プライム上場企業の" / "プライム市場上場企業の":
#     「の」で終わる表現はHPが「親会社の子会社」として自己紹介する際の語法。
#     上場本体の自己紹介（「東証プライム市場に上場しております」）とは異なる。
# - "上場企業の100%子会社":
#     ※仮置き。親会社の市場区分（プライム/スタンダード/グロース）は
#       判別できないが、上場の信用力を考慮して許容。
#       運用データで商談化率が低いと判明した場合は別タスクで分離する。
# - 具体的な親会社名12社: 東証プライム上場企業として確認済みの社名。
#     HPに親会社名が含まれる場合に確実にプライム子会社と判定できる。
PARENT_PRIME_KEYWORDS = [
    # プライム市場上場を明示する表現（語尾「の」で「親会社を指している」ことを示す）
    "東証プライム上場の",
    "プライム上場企業の",
    "プライム市場上場企業の",
    # 上場は確認できるが市場区分不明（仮置き・運用データで見直す可能性あり）
    "上場企業の100%子会社",
    # 具体的な親会社名（東証プライム上場確認済み）
    ...
    "GMOインターネットグループ",
    # ── 削除したキーワード（理由を残す） ──
    # "100%子会社"  → 非上場親会社の子会社も True になる
    # "完全子会社"  → 同上
    # "グループ会社" → スタンダード・グロース・非上場グループも True（最も広すぎる）
    # "の連結子会社" → 上場区分を問わない
    # "の関連会社"   → 持分20-50%の関連会社（子会社ではない）も True になる
]
